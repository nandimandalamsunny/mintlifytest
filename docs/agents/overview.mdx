...
*   **Workflow Fit**: Operates during the initial **Intent → Build** phase to ensure engineering work is grounded in clear requirements.
...
Revolte manages the lifecycle of a change through a coordinated handoff between specialized systems. This orchestration ensures that no change is implemented without proper planning or delivered without rigorous validation.
...
<CardGroup cols={2}>
  ...
  <Card title="Security Agent" icon="shield-check">
    **Compliance & Security** | Performs continuous vulnerability analysis, secure deployment checks, and ensures changes adhere to organizational security policies.
  </Card>
...
*   **Delivery Confidence**: Ensure that every pull request has been semantically reviewed and validated against your specific repository context.
