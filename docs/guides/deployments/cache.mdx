...
This guide explains how to configure a standalone Cache service in Revolte. For applications requiring high-performance data retrieval, Revolte also offers combined presets:
- **Web Service + Cache**
- **Web Service + Database + Cache**

<Steps>
  <Step title="Choose a Preset">
    Select the **Cache** tile for a standalone managed Redis instance, or choose a combined preset for integrated deployments.
    
    > **Note:** For combined presets, the core configuration workflow is identical to a standard [Web Service](/docs/guides/deployments/web-services). This guide focuses specifically on the supplemental **Cache** settings described below.
  </Step>
